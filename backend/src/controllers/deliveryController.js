...
export const listDeliveries = asyncHandler(async (req, res) => {
  ...
  const stats = {
    active: await Delivery.countDocuments({ status: { $in: ['On Route', 'Delayed', 'Rerouting'] } }),
...
export const createDelivery = asyncHandler(async (req, res) => {
  const { originLabel, destinationLabel, district, driver, vehicle, etaMinutes } = req.body;
...
export const updateDeliveryStatus = asyncHandler(async (req, res) => {
  ...
  const validStatuses = ['Scheduled', 'On Route', 'Delayed', 'Rerouting', 'Delivered'];
